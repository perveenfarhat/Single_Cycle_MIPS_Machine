...
assemble:
	...
	/media/Storage1/misc_work/problem/spimsimulator-code/spim/spim -full_dump -noexception -file $(INPUT);
	awk '{print $$2}' mem.data.dat |sed 's/0x//g'  >$(OUTPUT)/sim/mem.data.dat;
	awk '{print $$2}' mem.kdata.dat |sed 's/0x//g'  >$(OUTPUT)/sim/mem.kdata.dat;
	    awk '{print $$2}' mem.stack.dat |sed 's/0x//g' >$(OUTPUT)/sim/mem.stack.dat;
	    awk '{print $$2}' mem.text.dat |sed 's/0x//g'  >$(OUTPUT)/sim/mem.text.dat;
	    awk '{print $$2}' mem.ktext.dat |sed 's/0x//g'  >$(OUTPUT)/sim/mem.ktext.dat;
		#rm -f mem*dat;
